...
if grep -RInE '^\s*until\b|while[[:space:]]+true' \
    scripts/configure-sonarr.sh scripts/configure-radarr.sh scripts/configure-prowlarr.sh scripts/configure-qbittorrent.sh >/dev/null; then
    echo "Error: unbounded bootstrap loop reintroduced" >&2
    exit 1
fi

grep -q 'scripts/wait-for-services.sh' scripts/configure.sh
grep -q 'profiles: \["runtime"\]' compose.yaml
grep -q 'security coordinator LAST' run.sh
grep -q '/api/v2/app/preferences' coordinator/coordinator.py
if grep -q 'cookie.name == "SID"' coordinator/coordinator.py; then
    echo "Error: hard-coded qBittorrent SID cookie assumption reintroduced" >&2
    exit 1
fi
if grep -q 'has_session_cookie' scripts/configure-qbittorrent.sh; then
    echo "Error: qBittorrent bootstrap still depends on a cookie name" >&2
    exit 1
fi

grep -q 'QBITTORRENT_AUTH_RETRY_SECONDS' coordinator/coordinator.py
grep -q 'CHANGE_THIS_QBITTORRENT_PASSWORD' .env.example

a="$(grep -n 'scripts/configure-qbittorrent.sh' scripts/configure.sh | head -n1 | cut -d: -f1)"
b="$(grep -n 'scripts/configure-sonarr.sh' scripts/configure.sh | head -n1 | cut -d: -f1)"
(( a < b )) || {
...
for service in sonarr radarr prowlarr qbittorrent file-security; do
    grep -q "$service" scripts/wait-for-services.sh || {
